...
Provisions the AKS cluster and tier-isolated networking an Azure Argo data
plane runs on. Stage is the cluster's default node pool with its own
subnet; prod is a separate, tainted node pool with its own subnet and NSG
that explicitly denies inbound from the stage subnet. Raw `azurerm_*`
resource blocks, no dependency on any other WSO2 module repo.
...
- Optional etcd secrets encryption via a Key Vault + key
  (`enable_secrets_encryption` - must be turned on in a follow-up apply
  after the cluster already exists, since the cluster's own
  system-assigned identity can't be granted Key Vault access before it
  exists).
- Optional NSG Flow Logs (`enable_vpc_flow_logs`) to a dedicated storage
  account - note Azure retired *new* NSG flow log creation as of
  2025-06-30 (existing ones keep working until 2027-09-30); this module
  hasn't yet migrated to VNet flow logs (Network Manager), so this
  currently 400s on any fresh environment regardless of the setting.
...
- Per-env Workload Identity Federation identities for pipeline pods
  ...
  ServiceAccount)` subject. This module only creates the identity; actual
  permissions are granted separately via `deploy_identity_role_assignments`
  (gated on `create_role_assignments`, since granting Azure RBAC roles
  needs `Microsoft.Authorization/roleAssignments/write`, which a plain
...
| `deploy_identities` | `map(object({ namespace, service_account_name }))` | `{}` | Per-env Workload Identity Federation identities for pipeline pods. Permissions granted separately via `deploy_identity_role_assignments` |
...
| `enable_secrets_encryption` | `bool` | `false` | Enable AKS's etcd secrets encryption via a Key Vault + key. Can only be turned on in a follow-up apply after the cluster already exists |
| `log_retention_in_days` | `number` | `90` | |
| `enable_vpc_flow_logs` | `bool` | `false` | Whether to create NSG Flow Logs for the stage/prod NSGs |
